...
int main() {
    ...
    const auto &q1 = std::get<std::vector<int32_t>>(first.columns[1].data);
    assert(p1[0] == 10.5f && p1[1] == 20.0f);
    assert(q1[0] == 3 && q1[1] == 4);
    assert(!finished);
    ...
    const auto &q2 = std::get<std::vector<int32_t>>(second.columns[1].data);
    assert(p2[0] == 15.25f && p2[1] == 30.0f);
    assert(q2[0] == 2 && q2[1] == 5);
    assert(finished);
